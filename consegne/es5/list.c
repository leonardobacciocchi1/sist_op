...
ItemType* FindFirstAvailable(LIST l, int quantita) {
    LIST tmp = l;
    while (!isEmpty(tmp)) {
        if (tmp->item.num_min <= quantita && tmp->item.num_max >= quantita) {
            return &tmp->item;  // Restituisce il primo elemento valido trovato
        }
        tmp = tmp->next;
    }
...
ItemType* Findthecoppia(LIST l,LIST l2, int quantita){
    LIST tmp = l;
    int s = quantita;
    int sock;
    
    while (!isEmpty(tmp)) {
        s = s + tmp->item.num_min;
        sock = tmp->item.socket;
        ItemType* found_guida = FindFirstAvailable(l2, s);
        if (found_guida != NULL) {
            
            //tmp = Dequeue(l,tmp->item);
            return found_guida;  // Restituisce il primo elemento valido trovato
...
LIST EnqueueOrdered(LIST l, ItemType item) {
    NODE* new_node = createNode(item);
    if (l == NULL || l->item.num_min >= item.num_min) {
        new_node->next = l;
        return new_node;  // Inserisce in testa se la lista è vuota o l'elemento va in testa
    }

    NODE* temp = l;
    while (temp->next != NULL && temp->next->item.num_min < item.num_min) {
        temp = temp->next;
    }
